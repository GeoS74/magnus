...
38) надо понять где грань между габаритным и негабаритным местом
39)ЖелДорЭкспедиция позволяет получить список городов и доступ к калькулятору без токена
40) Типы доставки ЖДЭ
    { type: 1, name: 'Доставка сборных грузов', description: '' },
    {
        type: 2,
        name: 'Экспресс доставка грузов',
        description: 'Экспресс доставка грузов осуществляется автомобильным транспортом по строго установленному графику. Невозможна экспресс доставка с соблюдением теплового режима.'
    },
...
44) BUG DETECTED: в запросе ЖелДорЭкспедиции при указании города не срабатывает Челябинск, чтобы расчёт сработал надо указать что это город "г. челябинск"
    причём название города в примере из документации API пишется с маленькой буквы, для подстраховки название города приводитсяк нижнему регистру
    Надо проработать этот вопрос и изменить mainHandbookPlaces, добавив туда тип населенного пункта.
    Также не понятно как поведёт себя API если расчитывать пгт, деревни и т.д.
45) Решено: Почта России требует указать код типа упаковки, надо проработать установку правильной упаковки в зависимости от габаритных размеров (коробка M, коробка X, коробка XL)
    стандартные размеры коробок Почты России:
    Тип S: 260×170×80 мм.
    Тип M: 300×200×150 мм.
    Тип L: 400×270×180 мм.
    Тип XL: 530×360×220 мм.
    В описании API есть код 99 - нестандартная упаковка, но расчёт с этим кодом выдаёт ошибку
...
Решено -    Почта России
            ...
            Адамос Логистик
            Возовоз
Решено -    Луч (только для Челябинской области)
...
4) расширить список городов, добавив пгт, деревни и сёла
